...
def plotAModelCart(m,ax,X,Y,showAxis,minX,maxX,minY,maxY,title,sig):
    ...
    vaRssi = np.array(m[5]).reshape(width, height)
    vaRssi = vaRssi.astype(float)

    detections = np.array(m[3]).reshape(width, height)

    vaRssi[detections == 0] = -76
    ...
    reescaledAVR = scipy.ndimage.filters.gaussian_filter(vaRssi, sigma=sig,mode='reflect')
...
def plotAModelPolar(m,ax2,R,T,showAxis,maxR,maxT,title):
    width = m[2].info.width
    height = m[2].info.height
    realMin=-4345721.1692759292
    realMax = 463.33333333333337

    stdRssi = np.array(m[5]).reshape(width, height)
    detections = np.array(m[3]).reshape(width, height)

    stdRssi = stdRssi.astype(float)
    stdRssi[detections == 0] = float('nan')
    ...
    reescaledAVR = stdRssi - (realMin) + 1
...
if __name__ == '__main__':
    ...
    for m in modelList[:-1:]:
        ...
        plt.show()
        pdf = PdfPages('./figures/stdRSSI_cart_' + str(f) + '.pdf')
        pdf.savefig(figt)
        pdf.close()
